...
public class p3 {
  ...
  @Test
  public void test() {
    System.out.println(wordCount(ArrayUtil.convertToStringArray("[\"ant\",\"act\",\"tack\"]"),
        ArrayUtil.convertToStringArray("[\"tack\",\"act\",\"acti\"]")));
  }
  ...
  public void test2() {
    System.out.println(
        wordCount(
            ArrayUtil.convertToStringArray("[\"g\",\"vf\",\"ylpuk\",\"nyf\",\"gdj\",\"j\",\"fyqzg\",\"sizec\"]"),
             ArrayUtil.convertToStringArray("[\"r\",\"am\",\"jg\",\"umhjo\",\"fov\",\"lujy\",\"b\",\"uz\",\"y\"]")));
  }
  ...
  public int wordCount(String[] startWords, String[] targetWords) {
    ...
    for (String startWord : startWords) {
      List<Character> startCharList = startWord.chars().mapToObj(c -> (char) c)
          .collect(Collectors.toList());

      for (String targetWord : targetWords) {
        if (exits.contains(targetWord)) {
           continue;
        }
        if (startWord.length() == targetWord.length()-1) {
          List<Character> targetCollect = targetWord.chars().mapToObj(c -> (char) c)
              .collect(Collectors.toList());
          if (startCharList.contains(targetCollect.removeAll(startCharList))) {
              continue;
          }
          if (targetCollect.size()!=1) {
             continue;
          }


          System.out.println("监测。。。。");
          System.out.println("初始::" + startWord);
          System.out.println("目标::" + targetWord);
          exits.add(targetWord);
          count++;
        }
      }
